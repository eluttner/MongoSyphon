...
public class DocumentGenerator {
    ...
    public void runConversion() {
        ...
        while (doc != null) {
            ...
            if (currcount - lastcount == 1000) {
                ...
                if (targetMode.equalsIgnoreCase("subsection") == false) {
                    System.out
                            .printf("%d records converted in %d seconds at an average of %d records/s\n",
        ...
        if (targetMode.equalsIgnoreCase("subsection") == false) {
            System.out
                    .printf("%d records converted in %d seconds at an average of %d records/s\n",
                            currcount, mstime / 1000L, (1000L * currcount)
                                    / (mstime + 1));
        }
        if (mongoTarget != null) {
            mongoTarget.close();
    ...
    public Document getNext() {
        ...
        if (connection.hasResults() == false) {
            ...
            if (connection.getType() == "SQL") {
                ...
                connection.RunQuery(section.get("query", Document.class)
                        .getString("sql"), paramArray, params);
            ...
            if (targetType == "float") {
                if (o.getClass() == BigDecimal.class) {
                    ...
                    throw new UnsupportedOperationException(String.format(
                            "Mapping from %s to float is not supported.",
                            o.getClass()));
                }
            } else {
                throw new UnsupportedOperationException(String.format(
                        "Unsupported type mapping: %s", targetType));
            }
        }
    }
